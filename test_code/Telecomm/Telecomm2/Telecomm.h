...
/* 
  ...
  The handling of state is best done over another file, and the formatting of the information is handled in Formatter.h, which
...
class Telecomm {
  public: 
   ...
   // new Telecomm("192.168.1.117",5005,5005) is pi side, Telecomm("192.168.1.50",5005,5005) is laptop side
    Telecomm(std::string dst_addr, int dst_port, int src_port, std::string name = "");
    ~Telecomm();

    // In loop, call update; if != 0, error
    int update();

    // Can add other UDP communications, refered to by 'name'
    // If no name is given, assumed to be the main initial connection in w
